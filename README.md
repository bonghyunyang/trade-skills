...
감으로 찍지 말고 숫자로 정해요.
품목 이름만 알려주면 됩니다. HS코드도, API 키도, 회원가입도 없어요.
...
**Settings → Capabilities → Skills** 에 올리면 끝이에요.

인터넷만 되면 됩니다. 어디 가입할 일도, 키 받을 일도 없어요.
...
HS코드는 몰라도 괜찮아요. **품목 이름을 한국어로 말하면** 알아서 코드 후보를 찾아주고
"이 코드로 볼까요?" 하고 한 번 물어봐요. 그러고 나서 80초쯤 기다리면 답이 나옵니다.
...
여기에 나라별 경쟁사 표, 엑셀로 바로 열리는 CSV 3개, 한국어 리포트까지 같이 나와요.

**여기서 제일 쓸모 있는 건 두 CAGR을 나란히 놓고 보는 거예요.**

| 이런 모양이면 | 이런 뜻입니다 |
|---|---|
| 시장 ↑ · 우리 수출 ↓ | **점유율을 뺏기는 중** |
| 시장 ↓ · 우리 수출 ↑ | **중계무역 경유 가능성** (실제 소비지가 다른 나라) |
| 시장 ↑ · 우리 수출 ↑↑ | **점유율을 뺏어오는 중** — 지금 밀어야 할 곳 |

## 🆚 원래 이렇게 하셨다면

| | 지금까지 | trade-skills |
|---|---|---|
| 시장 하나 보려면 | KOTRA · K-stat · Trade Map 각각 열고 엑셀로 옮기기 (반나절) | 질문 한 줄, 80초 |
| HS코드 | 직접 찾아서 입력 | "화장품"이라고 말하면 끝 |
| 나라 여러 개 | 하나씩 다시 반복 | 기본 10개국 한 번에 |
| 성장률 | 우리 수출 실적만 보임 | **시장 성장률까지 나란히** |
| 경쟁 구도 | 따로 찾아봐야 함 | 1위 공급국·과점 여부 자동 표시 |
| 해석 함정 | 알아서 조심해야 함 | **리포트가 먼저 알려줌** |

## 🤔 왜 만들었나

해외영업 하면서 이게 제일 답답했어요.

**"어느 나라부터 뚫을지" 하나 정하는 데 시간이 너무 많이 들었어요.** 구글링부터 시작해서
무료든 유료든 보고서를 이것저것 뒤지고 정리하다 보면 시간이 훅 갑니다.

그렇게 해놓고도 근거를 대라고 하면 "경쟁사가 저기서 잘하고 있으니까",
"누가 시장 크다고 하던데" 같은 카더라뿐이었어요. 결국 출장 가서 직접 보고 나서야
맞춰가는 식이었는데, 그 과정이 참 싫었습니다.

그래서 **"어느 시장부터 갈지"를 숫자로 정하는 것** 딱 하나에 집중했어요.

사실 숫자만 뽑아주는 도구는 이미 많아요. 진짜 문제는 그 숫자가 **조용히 엉뚱한 결론으로**
데려가는 경우거든요. 수입 점유율을 시장 점유율로 착각한다든가, 중계무역 허브를
유망 시장으로 본다든가, 데이터 없는 나라를 나쁜 시장으로 읽는다든가.
...
현지 공장에서 만들어 파는 물량은 이 통계에 **아예 안 잡혀요.** 미국은 현지 화장품
회사가 워낙 강해서(P&G, Estée Lauder 같은) 실제 시장 점유율은 훨씬 낮습니다.
식품·자동차·철강·화장품처럼 현지에서 많이 만드는 품목일수록 이 차이가 커져요.
...
이번에 같이 조회한 나라들 사이에서만 매긴 **상대 점수**예요. 상관없는 나라 하나를
넣고 빼는 것만으로 순위가 뒤집히기도 해요. 판단이 애매하면 비교 대상을 바꿔서
두세 번 돌려보고, **그래도 순위가 안 흔들리는 나라**를 믿으세요.

### 3. "점유율 여유"가 크다고 빈 시장이 아닙니다

우리 점유율이 낮다고 자리가 빈 게 아니에요. 이미 다른 나라가 앉아 있을 수 있거든요.
...
한국은 관세법상 수출입 신고정보가 비밀유지 대상이라, 어느 회사가 뭘 얼마에
수출했는지가 어디에도 공개되지 않아요. 관세청도, 무역협회도 마찬가지입니다.
다른 무료 도구 찾아 헤매지 않으셔도 돼요.

기업 단위 데이터는 **선하증권(B/L)을 공개하는 나라**에서만 나와요. 미국이나 인도 같은 곳이요.
Panjiva · ImportYeti · Volza 같은 유료 서비스를 씁니다. 한국·EU·일본·중국은 이 방법으로도 안 나와요.

**수출 절차나 관세, 인증은 이 도구가 확인해준 게 아니에요.** Claude가 아는 일반 지식으로
답은 하겠지만 검증된 내용은 아닙니다. 특히 관세율은 2025년 들어 계속 바뀌고 있어서
