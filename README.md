...
- **HS 4자리는 이질적인 제품이 한 칸에 섞입니다.** 예를 들어 3907 안에는 폴리아세탈,
  에폭시, 폴리카보네이트, PET가 전부 들어 있어서 "우리 수지"의 시장이 아니라 그 묶음의
  시장이 나옵니다. 품목이 특정되면 6자리로 좁혀 다시 보는 편이 정확합니다.
- **표 안에서 연도가 섞일 수 있습니다.** 한국 수출액은 최신 연도가 나오는데 상대국이
  신고한 시장 규모·점유율은 한 해 전인 경우가 흔합니다. 리포트가 어느 해 기준인지
  밝히지만, 표만 떼어 인용할 때는 이 점을 같이 옮기셔야 합니다.
...
**대신 "어느 나라로 바이어를 찾으러 갈지"는 정해 드립니다.** 바이어 발굴의 앞 단계이고,
이걸 건너뛰면 아무 나라에나 콜드메일을 뿌리게 됩니다. 그래서 "바이어 어떻게 찾나요"라고
물으셔도 됩니다 — 못 드리는 것부터 말씀드린 뒤 나라 선정으로 넘어갑니다.
...
숫자를 파는 도구라서, 무엇을 어떻게 확인했는지도 같이 적어 둡니다.

**확인한 것**

- **관세청 공표치와 대조합니다.** 2024년 연간 총수출 683,764백만 달러(관세청) 대
  683,127,018,292(이 도구) — 차이 0.09%. HS 3304는 자리수까지 맞습니다. 이 대조는
  매주 월요일 CI에서 자동으로 돌고, 어긋나면 이슈가 열립니다.
- **오프라인 168개 + 라이브 계약 테스트.** 응답의 모양이 바뀌면 주간 검사가 잡습니다.
- **총수입이 부풀려지는 사고를 실제로 잡은 적이 있습니다.** v0.2.7까지는 한 나라의
  내역행이 응답 상한을 채우면 합계가 최대 5배까지 뛰었습니다(슬로베니아 HS3304
  $6.19억 → 실제 $1.24억). v0.3.0에서 원인을 제거했고, 같은 종류가 다시 생기면
  파트너 합계와 총계 대조에서 걸립니다.
- **트리거 문장 31개를 실제 세션에 그대로 던져 확인했습니다.** 그중 12개는 공개 Q&A에서
  실제 사용자가 쓴 문장을 원문 그대로 가져온 것입니다.

**아직 확인 못 한 것**

- **외부 사용자 검증이 없습니다.** 지금까지 확인은 만든 사람이 했습니다.
- **Claude Code에서만 확인했습니다.** Cowork·claude.ai는 스킬 선택 방식이 달라
  트리거가 같다는 보장이 없습니다.
- 관세율(WITS)은 참고용입니다. 실제 통관에 쓰기 전에 반드시 따로 확인하세요.

이상한 숫자를 보시면 [이슈](https://github.com/bonghyunyang/trade-skills/issues)로
알려주세요. 재현할 수 있는 형태면 테스트로 박아 두겠습니다.
...
./tests/run_tests.sh               # 오프라인 151개, 네트워크 불필요
